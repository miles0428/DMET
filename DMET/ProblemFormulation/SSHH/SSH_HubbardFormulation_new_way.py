...
class OneBodySSHHFormulation(OneBodyProblemFormulation):
    ...
    def get_slater(self, number_of_electrons):
        ...
        for _ in range(number_of_electrons - len(self._wavefunction) if number_of_electrons > len(self._wavefunction) else 0):
            if len(self._wavefunction) == 0:
                chosen_index = self.idx[0]
                self._wavefunction.append(self.eigenvectors[:, chosen_index])
                ...
                self._wavefunction.append(self.eigenvectors[:, self.idx[chosen_index]])
        ...
        wavefunction = np.column_stack(self._wavefunction[:number_of_electrons])
    ...
    def calculate_U_from_wavefunction(self, wavefunction):
        ...
        L = self.N_cells * 2
        U_energy = 0.0
        for wave_const in self._wavefunction:
...
if __name__=="__main__":
    ...
    # --- One-body problem ---
    onebody = OneBodySSHHFormulation(N_cells=N_cells, t1=t1, t2=t2, U = 0.1,number_of_electrons=number_of_electrons, PBC = True)
