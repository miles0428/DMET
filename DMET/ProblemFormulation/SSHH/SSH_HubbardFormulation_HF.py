...
class OneBodySSHHFormulation_HF(OneBodySSHHFormulation):
    def __init__(self, N_cells, t1, t2, U, number_of_electrons, PBC=True, alpha=0, tol=1e-8, max_iter=10000, initial_density=None, **kwargs):
        super().__init__(N_cells, t1, t2, U, number_of_electrons, PBC)
        self.U = U
        self.tol = tol
        self.max_iter = max_iter
        self.density_matrix = None
        self.initial_density = initial_density  # Store initial density for HF
    ...
    def get_density_matrix(self, hf_update_rate=0.1, initial_density=None):
        """
        Compute or retrieve the HF one-body density matrix for DMET.
        If HF has not been run yet, run it automatically.
        If initial_density is provided, use it as the starting point for HF.
        """
        # if self.density_matrix is None:
        self.run_hf(alpha=hf_update_rate, initial_density=initial_density)
